...
"""
量化选债历史数据回填 - 7月9日
直接版本，使用pymysql避免DataService初始化开销
"""
...
def apply_conditions(df, schemes):
    """应用条件筛选"""
    matches = []
    stats = {}
    seen = {}
...
def calculate_entry_price(signal_price, price_offset, offset_mode):
    """计算入场价"""
...
def main():
    ...
    with engine.connect() as conn:
        df_all = pd.read_sql(text(f"""
            ...
            ORDER BY time
        """), conn, params={'start': TIME_START, 'end': TIME_END})
    
    groups = list(df_all.groupby('time'))
    print(f"      共 {len(groups)} 个tick时间点，{len(df_all)} 条记录")
    ...
    for i, (tick_time, df) in enumerate(groups):
        tick_time_str = str(tick_time).replace(':', '')
        
        matches, stats = apply_conditions(df, schemes)
        ...
        if (i + 1) % 100 == 0:
            if batch_data:
                ...
                batch_data = []
            print(f"      进度: {i+1}/{len(groups)} ticks, 命中: {total_matches} 条")
