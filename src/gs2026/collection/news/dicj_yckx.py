...
def collect_rmcx():
    with sync_playwright() as p:
        ...
        try:
            ...
            page.goto("https://www.yicai.com/brief/",timeout=60000)
            time.sleep(1)

            all_news_data = []

            for i in range(1, 16):
                header_elements = page.query_selector_all(f'#onlist > ul:nth-child(4) > li:nth-child({i})')
...
def extract_news_from_text(news_text, separator=" | "):
    ...
    try:
        ...
        publish_time_ = first_line[:time_end] if time_end != -1 else ""

        # 提取标题和内容
        remaining = first_line[time_end + 1:] if time_end != -1 else first_line
        separator_pos = remaining.find(separator)

        if separator_pos != -1:
            title_ = remaining[:separator_pos]
            # 第一行的内容部分
            first_line_content = remaining[separator_pos + len(separator):]
            ...
            if len(lines) > 1:
                ...
                for line in lines[1:]:
                    # 遇到股票代码行时停止（简单判断）
                    if line.strip() and not any(indicator in line for indicator in ["SH", "SZ", "%"]):
                        content_lines.append(line.strip())
                    else:
                        break
                content_ = "\n".join(content_lines)
            else:
                content_ = first_line_content
        else:
            title_ = remaining
            content_ = ""
