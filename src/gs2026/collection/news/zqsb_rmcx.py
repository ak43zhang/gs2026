...
def collect_rmcx():
    with sync_playwright() as p:
        ...
        try:
            ...
            # 打开 i问财 页面
            page.goto("https://www.stcn.com/article/list/kx.html",timeout=60000)
            time.sleep(1)

            all_news_data = []

            for i in range(1, 11):
                header_elements = page.query_selector_all(f'body > div.quick-news-list-wrapper > div.w1200.m-auto.list-body > div.left > ul > li:nth-child({i})')
                for header in header_elements:
                    collect_content = header.inner_text()
                    news_data = extract_news_from_text(collect_content)
...
def extract_news_from_text(news_text):
    """
    从单个新闻文本中提取五个字段
    news_text格式示例：
    16:14
    ...
    try:
        ...
        # 2. 提取标题（第二行）
        title_ = lines[1]  # "恒生指数收涨0.05% 恒生科技指数跌1.84%"
        # 3. 提取内容（第三行），清理可能的尾随空格
        content_ = lines[2].strip()
