...
from gs2026.analysis.worker.message.stepfun.stepfun_client import (
    stepfun_analysis, StepfunClient, MODELS
)
...
def stepfun_ai(
    ...
    for item in query_list:
        ...
        # 调用阶跃API（核心替换）
        logger.info(f"开始阶跃分析: {t_date} {main_area}-{child_area}")
        try:
            analysis: str = stepfun_analysis(query, _headless)
        except Exception as e:
            logger.error(f"阶跃API调用异常: {e}")
            continue
        
        if not analysis or analysis.strip() in ('', '{}'):
            logger.error(f"阶跃返回空结果: {t_date} {main_area}-{child_area}")
            continue
        ...
        if string_util.is_valid_json(json_data) and json_data != '{}':
            ...
            try:
                stats = process_domain(
                    json_data, main_area, child_area, t_date, version='stepfun-1.0.0'
                )
                logger.info(f"拆分入库完成: {stats}")
            except Exception as e:
                logger.error(f"拆分入库失败: {e}")
        else:
            logger.error(f"JSON解析失败: {table_name}")
    
    elapsed = time.time() - start
    logger.info(f"{table_name} 分析完成，耗时: {elapsed:.2f}秒")
...
def area_ai_analysis(
    ...
    for cand in candidates:
        ...
        lock_key = f"area_ai_lock:stepfun:{table_name}:{t_date}:{main_area}:{child_area}"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='事件驱动分析-阶跃版')
