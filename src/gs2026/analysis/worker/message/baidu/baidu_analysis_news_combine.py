...
def get_news_combine_analysis(table_name,analysis_table_name,_headless):
    sql = f"select SQL_NO_CACHE `内容hash`,`内容` from {table_name} where  analysis is null or analysis=''  order by `发布时间` desc limit 20"
    with engine.connect() as conn:
        lists = pd.read_sql(sql, con=conn).values.tolist()
        if len(lists) >= 15:
            sample_list = random.sample(lists, random.randint(8, 12))
            baidu_ai(sample_list, table_name,analysis_table_name,_headless)

def time_task_do_combine(polling_time):
    while True:
        get_news_combine_analysis("news_combine","analysis_news",True) #
        time.sleep(polling_time)

if __name__ == "__main__":
    run_daemon_task(target=time_task_do_combine, args=(3,))
