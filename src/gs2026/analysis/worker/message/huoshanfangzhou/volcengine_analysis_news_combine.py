...
def volcengine_ai(
    ...
    # ── 构造完整 Prompt（与DeepSeek版一致，综合评分×5） ─────────────────────
    query = query + f"""
                    ...
                    重要程度评分：
                        ...
                        按照 相关性与纯度 角度评估程度分为 标的核心业务与消息强绑定，形成量价齐升/份额提升/利润爆发等实质性利好，全产业链强传导受益（5分）、标的属于产业链上下游/存在间接联动影响/利好传导存在一定滞后（3分）、情绪相关（概念沾边）（1分），最终综合分析算出。
                    业务影响维度评分：（每个维度-5至5分，总分范围-60至60）
                        ...
                        按照 市场份额扩张 维度评估程度分为	外部供给缺失/需求激增/企业可大幅抢占国内外市场份额/行业格局重构（5）、外部环境利好/企业明显提升市场占有率（3）、依托自身优势,市场份额小幅提升（1）	大幅下降（-5）、明显下降（-3）、小幅下降（-1），
                        按照 产业链地位 维度评估程度分为	成为产业链核心刚需环节，行业话语权、议价能力大幅提升（5）、产业链话语权有所提升，行业地位巩固加强（3）、产业链地位轻微提升（1）	大幅降低（-5）、有所降低（-3）、轻微降低（-1），
                        ...
                        按照 行业趋势红利 维度评估程度分为	所处赛道处于市场爆发风口，行业景气度拉满（5）、明显受益（3）、略有受益（1）	逆势而行（-5）、明显受损（-3）、略有受损（-1），
                        按照 输入成本下降 维度评估程度分为	大幅降低原材料/能源成本（5）、明显降低（3）、小幅降低（1）	大幅上升（-5）、明显上升（-3）、小幅上升（-1），
                        最终综合分析算出。
                    综合评分：（通过重要程度评分×5+业务影响维度评分）。
...
def get_news_combine_analysis(table_name: str, analysis_table_name: str, _headless: bool = True) -> None:
    ...
    with engine.connect() as conn:
        ...
        if len(lists) < 30:
            logger.info("[火山方舟-聚合] 数据量小于20，暂不处理")
            return False
        if len(lists) >= 30:
