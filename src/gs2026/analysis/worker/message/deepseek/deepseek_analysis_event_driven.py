...
def deepseek_ai(
    ...
    for i in query_list:
        ...
        query = f"{t_date}全球重要大事件集锦，按重要程度给出30条主领域为{main_area}，子领域为{child_area}的消息，" + """
                    ...
                    深度分析：(是根据成本控制、运营效率、资金与财务、技术或工艺突破、产品定价权、市场份额扩张、产业链地位、产品结构升级、成功拓展新业务、政策支持、行业趋势红利、输入成本下降等多个维度分析该消息的实质性影响。
                    格式要求：深度分析必须是JSON数组格式（Array），包含12个字符串元素，每个元素格式为"维度名称：具体原因描述，+/-分数分"。
                    正确示例（必须使用此格式）：
                    [
                        "成本控制：换电模式降低初始购车成本，+3分",
                        "运营效率：3分钟换电大幅提升运营效率，+5分",
                        "资金与财务：新业务贡献营收，改善财务，+3分",
                        "技术或工艺突破：换电重卡技术明显进步，+5分",
                        "产品定价权：先发优势带来一定定价能力，+3分",
                        "市场份额扩张：在换电重卡市场取得领先份额，+5分",
                        "产业链地位：吉利在商用车电动化领域地位提升，+3分",
                        "产品结构升级：推出高端换电产品，+3分",
                        "成功拓展新业务：开辟换电商用车新业务，+5分",
                        "政策支持：无直接支持，符合环保趋势，+1分",
                        "行业趋势红利：重卡电动化风口，+5分",
                        "输入成本下降：无直接影响，0分"
                    ]
                    错误示例（严禁使用此格式）：
