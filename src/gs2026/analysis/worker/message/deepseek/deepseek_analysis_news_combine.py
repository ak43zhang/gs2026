...
def deepseek_ai(
    ...
    query = query + f"""
                    ...
                    其中，消息id字段只存一个id。
                    重要程度评分：按照 权威性与级别 角度评估程度分为 国家级政策（5分）、部委/地方政策（4分）、行业会议（3分）、公司公告（2分）、市场传闻（1分）。按照 新颖性与想象力 角度评估程度分为 新技术/新政策（5分）、现有产业数据向好（3分）。按照 相关性与纯度 角度评估程度分为 直接受益（核心业务高度相关）（5分）、间接受益（产业链上下游）（3分）、情绪相关（概念沾边）（1分），最终综合分析算出。
...
def get_news_combine_analysis(
    ...
    Args:
        ...
        _headless: 是否以无头模式运行浏览器。
    """
    # 查询未分析的消息，随机排列，限制 60 条
    sql: str = f"select SQL_NO_CACHE `内容hash`,`内容` from {table_name} where (analysis is null or analysis='') order by  RAND() limit 60"  # SUBSTRINg(`发布时间`,1,7) desc,
