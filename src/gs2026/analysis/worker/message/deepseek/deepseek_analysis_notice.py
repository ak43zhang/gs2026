...
from gs2026.analysis.worker.message.prompts import build_notice_prompt
...
def deepseek_ai(
    ...
    query = build_notice_prompt(query, count, notice_type_dic_str)
    # print(query)  # 如需调试可取消注释
    # 对Prompt中的敏感词进行替换处理
    query = string_util.sensitive_word_replacement(query)

    # 调用DeepSeek模型进行分析（process_name用于次数限制：登录时计数）
    analysis: str = deepseek_analysis_event_driven.deepseek_analysis(query, _headless, process_name="deepseek_notice")
                ...
                if not available:
                    ...
                    if not locked:
                        ...
                        try:
                            ...
                            deepseek_ai(items_to_analyze, notice_type_dic_str, 
...
def timer_task_do_notice_with_limit(polling_time: int, year: str = "2026", process_name: str = 'deepseek_notice') -> None:
    """带AI调用次数限制的公告分析轮询任务（非嵌入式包装器）。

    ★ 移除此功能：将 __main__ 中的 timer_task_do_notice_with_limit 改回 timer_task_do_notice 即可。
    """
...
if __name__ == "__main__":
    ...
    # ★ 使用带AI调用次数限制的版本（移除限制：改回 timer_task_do_notice 即可）
    run_daemon_task(target=timer_task_do_notice_with_limit, args=(polling_time, year), daemon=False)
