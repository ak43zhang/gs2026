...
try:
    exists = mysql_tool.table_exists('analysis_area2026')
    if exists:
        print("  [OK] 表存在")
    else:
        print("  [FAIL] 表不存在")
...
try:
    ...
    # 查询最后插入的ID
    print(f"\n查询最后插入的ID...")
    id_result = mysql_tool.query("SELECT LAST_INSERT_ID() as id")
    if id_result:
        last_id = id_result[0][0]
        print(f"  [主键ID]: {last_id}")
    else:
        print(f"  [WARN] 无法获取主键ID")
        last_id = None
    
    # 验证数据
    print(f"\n验证写入数据...")
    verify_sql = f"SELECT id, news_date, main_area, child_area, LENGTH(json_data) as json_len FROM analysis_area2026 WHERE id = {last_id}"
    verify_result = mysql_tool.query(verify_sql)
