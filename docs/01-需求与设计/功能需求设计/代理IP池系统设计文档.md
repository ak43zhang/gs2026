...
原方案30分钟0个代理，原因：
- 3000个代理全部验证DeepSeek（每个8秒×5轮=40秒）
- 海外IP无法访问DeepSeek，浪费时间
- 国内IP占比<5%，验证效率极低
...
| 第一层验证 | - | 900个存活 | 新增 |
| 第二层筛选 | - | 45个国内IP | 新增 |
| DeepSeek验证 | 3000×40s=33小时 | 45×25s=18分钟 | **99%↓** |
| 总耗时 | 30分钟+ | 5-10分钟 | **80%↓** |
| 入池代理 | 0-2个 | 10-20个 | **10倍↑** |

### 12.4 关键代码

```python
def validate_proxy_basic(self, proxy: ProxyInfo) -> bool:
    """第一层：基础验证（访问 httpbin.org）"""
    ...

def refresh(self):
    # 第一层：基础验证
    alive = self.validate_batch_basic(proxies)
    
    # 第二层：归属地筛选
    cn_proxies = [p for p in alive if self._geo_checker.get_country(p.ip) == 'CN']
    
    # 第三层：DeepSeek验证（只验证国内IP）
    valid = self.validate_batch(cn_proxies, quality_mode=True)
```
...
| 验证层 | URL | 超时 | 用途 |
|--------|-----|------|------|
| 基础验证 | http://httpbin.org/ip | 5秒 | 验证代理存活 |
| DeepSeek验证 | https://chat.deepseek.com/ | 5秒 | 验证能访问DS |

---

*文档创建时间: 2026-06-01 04:33*  
*最后更新: 2026-06-01 06:30*
