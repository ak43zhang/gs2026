...
```
□ 问题1：派生字段异步数据竞争 → 提交前先copy隔离（P0推荐）
□ 问题2：阶段3恢复统一走TickStateCache（P1推荐）
□ 问题3：清理_PREV_MAIN_CACHE遗留引用（P2）
□ 问题4：bond快照收集频率优化（P3可选）
□ 问题5：早盘基准逻辑抽象（P3可选）
□ 问题6：累计值继承行为（P3记录即可）
□ 问题7：debug日志清理（P4，排查后）
□ 问题8：MySQL备份表预检（P4可选）
□ 问题9：表结构检查抽象（P4可选）

请审核，确认修复哪些问题。建议至少修复 P0(问题1) 和 P2(问题3)。
```
