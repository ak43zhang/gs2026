...
def check_stock_sssj():
    ...
    if gp_df is not None and not gp_df.empty:
        ...
        # 查找华特科技
        huate = gp_df[gp_df['short_name'].str.contains('华特', na=False)]
        if not huate.empty:
            logger.info(f"\n华特科技数据:")
            logger.info(huate.to_string())
    else:
        logger.error("无法获取股票实时数据")
