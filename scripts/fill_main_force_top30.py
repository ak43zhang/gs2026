...
def main():
    print(f"开始填充 {TABLE_NAME} 的主力净额数据（Top 30股票优先）")
    print("=" * 60)
    
    engine = get_engine()
    
    # 获取15:00:00时排行榜中的股票（收盘排行）
    ...
    print(f"优先处理收盘排行榜 Top {len(top_stocks)} 股票")
    print()
    
    total_updated = 0
    
    for idx, stock_code in enumerate(top_stocks, 1):
        try:
            with engine.connect() as conn:
                # 获取该股票的所有数据
                df = pd.read_sql(
                ...
                print(f"{idx}. {stock_code}: 更新 {len(df_valid)} 条, 净流入 {inflow} 次, 净流出 {outflow} 次, 净额 {total_net:,.0f} 元")
                
        except Exception as e:
            print(f"{idx}. {stock_code}: [ERROR] {e}")
            continue
    
    print()
    print("=" * 60)
    print(f"[OK] Top 30 股票填充完成，共更新 {total_updated} 条记录")
    print()
