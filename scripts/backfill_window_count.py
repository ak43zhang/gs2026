"""
测试回填 monitor_gp_top30_20260618 表的 window_count 字段
"""
import sys
...
table_name = 'monitor_gp_top30_20260618'
date_str = '20260618'
...
def backfill_window_count():
    ...
    with engine.connect() as conn:
        ...
        if result.scalar() == 0:
            ...
            conn.commit()
            print(f"已添加 window_count 字段到 {table_name}")
        else:
            print(f"window_count 字段已存在")
    ...
    # 4. 计算 window_count
    # 内存缓存模拟：{(date, window_start, code): count}
    window_cache = {}
    ...
    # 6. 验证
    with engine.connect() as conn:
        verify_df = pd.read_sql(f"SELECT code, name, time, window_count FROM {table_name} WHERE window_count > 0 ORDER BY time DESC LIMIT 10", conn)
        print("\n验证数据（前10条window_count>0）：")
