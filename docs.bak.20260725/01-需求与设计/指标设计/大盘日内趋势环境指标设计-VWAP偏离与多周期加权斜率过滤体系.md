...
### 6.5 各指标前端字段说明与条件设置参考

> 以下字段在前端"量化回测 → 入场条件"中直接可选（属于"大盘趋势"分组），条件设置后即刻生效于回测和实盘监控。
...
| 前端分组 | 大盘趋势 |
| 数据类型 | float |
| 单位 | 百分比（如 0.5 表示大盘较开盘涨0.5%） |
| 典型范围 | -2.0 ~ +2.0 |
| 更新频率 | 每tick（3秒） |
| 含义 | 当前全市场可转债均价相对开盘的涨跌幅 |

**条件设置指南**：

| 目的 | 条件 | 说明 |
|------|------|------|
| 过滤大幅下跌 | `mkt_vs_open_pct > -0.3` | 大盘跌超0.3%禁止交易 |
| 只做上涨行情 | `mkt_vs_open_pct > 0` | 大盘必须为正才交易 |
| 过滤极端行情 | `mkt_vs_open_pct < 2.0` | 涨太多可能回调，不追高 |

---

#### `mkt_vwap_bias` — 大盘VWAP偏离
...
| 典型范围 | -0.5 ~ +0.5 |
| 更新频率 | 每tick（3秒） |
| 含义 | 当前大盘涨跌幅与成交额加权均价(VWAP)的偏离。正=在均价线上方，负=在均价线下方 |

**条件设置指南**：

| 目的 | 条件 | 说明 |
|------|------|------|
| **核心过滤：杜绝单边下行** | `mkt_vwap_bias > 0` | 价格在均价线上方才允许交易（最有效的单边下行过滤器） |
| 允许接近VWAP时交易 | `mkt_vwap_bias > -0.05` | 略低于均价线也可（适配震荡市） |
| 反转确认辅助 | `mkt_vwap_bias > -0.1` | 配合其他反转条件使用 |

**关键理解**：
- 单边下行日：价格全天在VWAP下方（bias持续为负），反弹触碰VWAP即回落
- 正常上涨日：价格大部分时间在VWAP上方（bias为正）
- **此指标是区分"下跌中继反弹"和"真实反转"的核心** — 真反转会突破VWAP并站稳

---

#### `mkt_weighted_slope_5m` — 大盘加权斜率(5min)
...
| 更新频率 | 每tick（3秒） |
| 延迟 | ~25秒确认方向翻转 |
| 含义 | 最近5分钟的大盘趋势方向（EWLR半衰期75秒），能过滤1-3分钟的假反弹 |

**条件设置指南**：

| 目的 | 条件 | 说明 |
|------|------|------|
| 中周期趋势确认 | `mkt_weighted_slope_5m > 0` | 5分钟方向向上才交易 |
| 更严格过滤 | `mkt_weighted_slope_5m > 0.0005` | 要求有明确上涨力度 |
| 排除强烈下跌 | `mkt_weighted_slope_5m > -0.001` | 允许弱势但排除急跌 |

**与2min斜率的区别**：
- `mkt_weighted_slope_2m`（半衰期30s）：捕捉瞬时动量，易被1-2分钟假反弹欺骗
- `mkt_weighted_slope_5m`（半衰期75s）：过滤短期噪声，反映真实中周期方向

---

#### `mkt_weighted_slope_10m` — 大盘加权斜率(10min)
...
| 更新频率 | 每tick（3秒） |
| 延迟 | ~50秒确认方向翻转 |
| 含义 | 最近10分钟的大盘大方向（EWLR半衰期150秒），不会被5分钟以内的反弹干扰 |

**条件设置指南**：

| 目的 | 条件 | 说明 |
|------|------|------|
| **大方向硬过滤** | `mkt_weighted_slope_10m > 0` | 大趋势必须向上 |
| 排除强烈空头 | `mkt_weighted_slope_10m > -0.0005` | 不强求向上，但不能强烈下行 |
| 极端保守 | `mkt_weighted_slope_10m > 0.0005` | 大盘必须有明确上涨态势 |

**适用场景**：
- 作为"总开关"使用：大盘10分钟趋势为负 → 直接禁止所有做多信号
- 单边下行日此值开盘后快速转负并持续整天

---

#### `mkt_weighted_slope_15m` — 大盘加权斜率(15min)
...
| 更新频率 | 每tick（3秒） |
| 延迟 | ~60-90秒确认方向翻转 |
| 含义 | 最近15分钟的大盘超大方向（EWLR半衰期180秒），最稳定，几乎不受短期波动影响 |

**条件设置指南**：

| 目的 | 条件 | 说明 |
|------|------|------|
| 超长周期确认 | `mkt_weighted_slope_15m > 0` | 15分钟大方向向上 |
| 与10m配合使用 | 10m > 0 AND 15m > -0.0003 | 中期向上且长期不强烈向下 |

**注意**：延迟较大（~60-90秒），适合作为辅助确认而非主过滤器。

---

#### `mkt_day_position` — 大盘日内位置(%)
...
| 更新频率 | 每tick（3秒） |
| 延迟 | 零 |
| 含义 | 当前大盘涨跌幅在今日最高与最低之间的位置。0%=在日内最低点，100%=在日内最高点 |

**条件设置指南**：

| 目的 | 条件 | 说明 |
|------|------|------|
| 不在底部区域交易 | `mkt_day_position > 30` | 至少从底部反弹了30%的日内振幅 |
| 脱离底部 | `mkt_day_position > 20` | 更宽松，允许底部稍上方入场 |
| 只在高位交易 | `mkt_day_position > 60` | 只在日内偏高位置做多（趋势延续） |
| 不追高 | `mkt_day_position < 90` | 避免在日内极值追入 |

**关键理解**：
- 单边下行日：此值始终接近0-20%（贴着日内底部运行）
- 反弹时：迅速从低位升高（如从10%升到50%）
- 震荡日：在30-70%区间波动
- **开盘前10分钟慎用**：日内振幅小时，此值波动大、不稳定

---

#### `mkt_new_low_distance` — 距新低分钟数
...
| 前端分组 | 大盘趋势 |
| 数据类型 | float |
| 单位 | 分钟（如 5.0 表示距上次创新低已过去5分钟） |
| 典型范围 | 0 ~ 240（一个交易日约4小时） |
| 更新频率 | 每tick（3秒） |
| 延迟 | 零 |
| 含义 | 距离大盘上一次创出日内新低的时间。值越大=越久没创新低=下跌可能停止 |

**条件设置指南**：

| 目的 | 条件 | 说明 |
|------|------|------|
| **反转第一信号** | `mkt_new_low_distance > 5` | 5分钟没创新低=下跌动能衰竭 |
| 快速判断停止下跌 | `mkt_new_low_distance > 3` | 3分钟没创新低（激进） |
| 确认下跌完全停止 | `mkt_new_low_distance > 10` | 10分钟没创新低（保守） |
| 正常上涨日默认满足 | `mkt_new_low_distance > 15` | 上涨日此值持续很大 |

**关键理解**：
- **这是识别"停止创新低"的核心指标**，零滞后
- 单边下行日：不断创新低，此值始终小（0-2分钟）
- 反转初期：停止创新低，此值开始增长（最早的反转信号）
- 正常上涨日：开盘后一直不创新低，此值持续很大
- **推荐作为反转确认的第一个条件**，配合VWAP偏离或5min斜率使用

---

### 6.6 前端条件选择器字段映射总表

> 前端 `quant_backtest.html` 条件选择器中的"大盘趋势"分组对应字段：

| 前端显示名 | 字段名(field) | 运算符(op) | 推荐值(value) | 说明 |
|-----------|--------------|-----------|--------------|------|
| 大盘涨跌幅(%) | `mkt_vs_open_pct` | `>` | -0.3 ~ 0 | 限制大盘跌幅 |
| 大盘VWAP偏离 | `mkt_vwap_bias` | `>` | -0.1 ~ 0 | 核心趋势过滤 |
| 大盘加权斜率(5min) | `mkt_weighted_slope_5m` | `>` | -0.001 ~ 0.0005 | 中周期方向 |
| 大盘加权斜率(10min) | `mkt_weighted_slope_10m` | `>` | -0.0005 ~ 0 | 大方向开关 |
| 大盘加权斜率(15min) | `mkt_weighted_slope_15m` | `>` | -0.0003 ~ 0 | 超长周期辅助 |
| 大盘日内位置(%) | `mkt_day_position` | `>` | 20 ~ 30 | 脱离底部 |
| 距新低分钟数 | `mkt_new_low_distance` | `>` | 3 ~ 10 | 停止创新低 |

---

### 6.7 快速上手：推荐条件配置

#### 场景一：我只想过滤掉单边下行，其他不影响
...
效果：停止创新低5分钟 + 中周期方向向上 + 接近均价线 → 反转确认后可交易。
